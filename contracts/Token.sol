...
contract SimpleToken is Ownable {
    ...
    function initialize(address[] memory addresses, uint256[] memory allocations) public onlyOwner {
        require(!initialized, "failure");
        require(addresses.length == allocations.length, "same array lengths");
        initialized = true;
    ...
    function transfer(address _to, uint256 _value) public returns(bool) {
        require(_to != address(0), "non 0 address");
        require(_value <= balances[msg.sender], "must have enough money");
